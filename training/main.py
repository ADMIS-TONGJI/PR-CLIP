...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.save_logs:
            ...
            metrics = evaluate(model if not args.model_ema else model_ema.ema, completed_epoch, preprocess_val, args, writer)

            if metrics[f"retrieval-mean-recall"] > best:
                best = metrics[f"retrieval-mean-recall"]
                if is_master(args):
                    torch.save(
                        checkpoint_dict,
                        os.path.join(args.checkpoint_path, f"best.pt"),
                    )
                    logging.info(f"Best model saved at epoch {completed_epoch} with retrieval-mean-recall {best}")
