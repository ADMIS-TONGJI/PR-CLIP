...
def retrieval_evaluation(model, epoch, preprocess, args, recall_k_list=[1,5,10]):
    ...
    if args.retrieval_data=='mscoco_captions_2014':
        ...
        from torchvision.datasets.coco import CocoCaptions
        coco_val_root = os.path.join(args.datasets_dir, 'val2017')
        coco_val_json = os.path.join(args.datasets_dir, 'annotations/captions_val2017.json')
        dataset = CocoCaptions(root=coco_val_root, annFile=coco_val_json, transform=preprocess)
    else:        
        dataset = CsvDataset(
            ...
            nori_dataset=False,
            images_dir=args.retrieval_images_dir
        )
