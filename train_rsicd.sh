...
port=$2
name=$3
CUDA_VISIBLE_DEVICES=$card torchrun --nproc_per_node 4 --master_port $port -m training.main \
    --train-data './dataset/train/ret2.csv' \
    --images-dir './images' \
    --csv-separator ',' --csv-img-key 'filename' --csv-caption-key 'title' \
    --retrieval-data './dataset/test/rsicd_test.csv' \
    ...
    --epochs 20 --save-frequency 0 --batch-size 160 --workers 2 \
    --lr 5e-6 --warmup 100 --weight_decay 0.5 --max-grad-norm 50 \
...
CUDA_VISIBLE_DEVICES=$card torchrun --nproc_per_node 2   --master_port $port -m training.main \
    --retrieval-data './dataset/test/rsicd_test.csv' --retrieval-images-dir './images' \
    --retrieval-csv-separator ',' --retrieval-csv-img-key 'filename' --retrieval-csv-caption-key 'title' \
    --retrieval-frequency 1 \
